...
<style>
  ...
  subtitle(data) {
    ...
    return `${servers[parseInt(data.server, 10)]} - ${
      itemList[parseInt(data.info, 10)].name
    }`;
  },
  ...
  action(cache) {
    ...
    switch (info) {
      ...
      case 2:
        result =
          this.dest(Audio.dispatchers[targetServer.id], "startTime") || 0; // when the music first started playing
        break;
      case 3:
        result =
      ...
      case 6:
        result =
          this.dest(
            Audio.dispatchers[targetServer.id],
            "streams",
            "opus",
            "_options",
            "rate"
