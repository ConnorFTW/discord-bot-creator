...
<div >
  ...
  init() {
    ...
    varName.oninput = function oninput() {
      ...
      conditions.style["padding-top"] = `${
        permissionsList[dataType].length * 66
      }px`;
    };
  ...
  action(cache) {
    ...
    permsArray.forEach((perms) => {
      if (data[perms] === "Allow") {
        ...
        permissions.allow.has(perms) || permissions.disallow.has(perms)
          ? no.push(perms)
          : yes.push(perms);
      }
    });
    ...
    if (storage2 && varName2 && no.length !== 0)
      this.storeValue(no, storage2, varName2, cache);
