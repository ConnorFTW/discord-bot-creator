...
<div>
  ...
  action(cache) {
    ...
    for (const command of this.getDBM().Files.data.commands) {
      if (
        command &&
        JSON.stringify(command.actions) === JSON.stringify(cache.actions)
      ) {
        cmd = command;
        break;
      }
  ...
  mod(DBM) {
      ...
      for (const command of Object.keys(Cooldown)) {
        ...
        if (Cooldown[command]) {
          if (!DBM.Bot.$cmds[command]) {
            ...
            const action = DBM.Bot.$cmds[command].actions.find(
              (a) => a.name === "Set Time Restriction"
            );
            if (action !== undefined) {
              if (action.save === "1") {
                delete Cooldown[command];
    ...
    DBM.Actions.TimeRestriction = function TimeRestriction(msg, cmd, cache) {
      ...
      const { Files } = DBM;
      let value = parseInt(
        this.evalMessage(cache.actions[cache.index].value, cache),
        10
      );
      ...
      switch (restrict) {
        case 0:
          ...
          if (Cooldown[cmd.name][msg.author.id]) {
            const expirationTime =
              Cooldown[cmd.name][msg.author.id] + Cooldown[cmd.name].cooldown;
            if (now < expirationTime) {
              return Math.ceil((expirationTime - now) / 1000);
            }
            Cooldown[cmd.name][msg.author.id] = now;
            if (Cooldown[cmd.name].save === 0)
              Files.saveGlobalVariable("DBMCooldown", JSON.stringify(Cooldown));
            return false;
          }
          Cooldown[cmd.name][msg.author.id] = now;
          if (Cooldown[cmd.name].save === 0)
        ...
        case 1: {
          ...
          if (Cooldown[cmd.name][msg.author.id][channelId]) {
            const expirationTime =
              Cooldown[cmd.name][msg.author.id][channelId] +
              Cooldown[cmd.name].cooldown;
            if (now < expirationTime) {
              return Math.ceil((expirationTime - now) / 1000);
            }
