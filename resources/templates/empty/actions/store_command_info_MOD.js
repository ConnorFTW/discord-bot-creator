  ...
  action(cache) {
    ...
    const command =
      parseInt(data.searchCommandBy, 10) === 0
        ...
        ? jp.query(
            this.getDBM().Files.data.commands,
            `$..[?(@._id=="${this.evalMessage(data.valueToSearch, cache)}")]`
          )
        : jp.query(
            this.getDBM().Files.data.commands,
            `$..[?(@.name=="${cache.msg.content
              .slice(
                this.getDBM().Files.data.settings.tag.length ||
                  cache.server.tag.length
              )
              .split(/ +/)
              .shift()}")]`
          );
    ...
    switch (parseInt(data.info, 10)) {
      case 0:
        result =
          jp.query(command, "$..name").length > 1
            ? jp.query(command, "$..name")[0]
            : jp.query(command, "$..name");
        break;
      case 1:
      ...
      case 3:
        ...
        break;
      case 4:
        result = JSON.stringify(jp.query(command, "$..permissions"))
          .slice(2, -2)
          .replace("_", " ")
          .toLowerCase();
        break;
