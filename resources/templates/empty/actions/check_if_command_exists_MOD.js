  ...
  action(cache) {
    ...
    if (commandName.startsWith(cache.server.tag)) {
      commandName = commandName
        .slice(cache.server.tag.length)
        .split(/ +/)
        .shift();
    } else if (commandName.startsWith(this.getDBM().Files.data.settings.tag)) {
      commandName = commandName
        .slice(this.getDBM().Files.data.settings.tag.length)
        .split(/ +/)
        .shift();
    }

    const commandsFile = JSON.parse(
      fs.readFileSync("./data/commands.json", "utf-8")
    );
    ...
    if (commandName === "")
      return console.log(
        "Please put something in 'Command Name' in the 'Check If Command Exists' action..."
      );
